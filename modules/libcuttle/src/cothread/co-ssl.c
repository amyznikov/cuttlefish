...
co_ssl_socket * co_ssl_tcp_connect(const struct sockaddr * address,  int tmo_ms, SSL_CTX * ssl_ctx)
{
  ...
  if ( (so = socket(address->sa_family, SOCK_STREAM, IPPROTO_TCP)) == -1 ) {
    CF_SSL_ERR(CF_SSL_ERR_STDIO, "socket(sa_family=%d) fails: %s", address->sa_family, strerror(errno));
    goto end;
  }
  ...
  if ( ssl_sock->ssl ) {
    ...
    co_socket_set_sndrcv_tmouts(cc, tmo_ms, tmo_ms);
...
co_ssl_socket * co_ssl_tcp_server_connect(const char * server, uint16_t port, int tmo_ms, SSL_CTX * ssl_ctx)
{
  ...
  if ( (status = co_resolve(server, &ai, &addrshints, tmo_ms > 0 ? tmo_ms * 1000 : -1)) ) {
  ...
  if ( !(ssl_sock = co_ssl_tcp_connect(ai->ai_addr, tmo_ms, ssl_ctx)) ) {
    goto end;
  }
...
co_socket * co_ssl_tcp_listen(const struct sockaddr * addrs)
{
  ...
  if ( (so = socket(addrs->sa_family, SOCK_STREAM, IPPROTO_TCP)) == -1 ) {
    CF_SSL_ERR(CF_SSL_ERR_STDIO, "socket(sa_family=%d, IPPROTO_TCP) fails: %s", addrs->sa_family, strerror(errno));
    goto end;
  }
